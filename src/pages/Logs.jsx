...
import {
  ...
  Eye, X, Save, XCircle, AlertCircle, Info, Send, Lock,
} from 'lucide-react';
...
/* ─────────────────────────────────────────────────────────────────
   Status reference
   ─────────────────────────────────────────────────────────────── */
// Draft          → volunteer working on it
// In Progress    → volunteer on-going activity
// Pending Review → volunteer submitted for TL approval
// Needs Revision → TL rejected, volunteer must fix and resubmit
// Completed      → TL approved
...
const STATUS_META = {
  ...
  'Needs Revision':  { label: 'Needs Revision',  badge: 'badge-error',    desc: 'Rejected — please update & resubmit' },
  'Completed':       { label: 'Completed',        badge: 'badge-success',  desc: 'Approved by Team Lead'            },
};

// Statuses a VOLUNTEER can set on their own log
const VOLUNTEER_STATUSES = ['Draft', 'In Progress', 'Pending Review'];
...
const TAGS = ['Planning', 'Docs', 'Meeting', 'Development', 'Testing', 'Review', 'Infra', 'Outreach', 'Other'];
const tagBadge = { Planning: 'badge-primary', Docs: 'badge-neutral', Meeting: 'badge-warning', Development: 'badge-secondary', Testing: 'badge-success', Review: 'badge-neutral', Infra: 'badge-neutral', Outreach: 'badge-secondary', Other: 'badge-neutral' };
...
/* ─────────────────────────────────────────────────────────────────
   Log Form Modal (Add / Edit) — for volunteers
   ─────────────────────────────────────────────────────────────── */
const LogFormModal = ({ initial, onClose, onSave, currentUser, role }) => {
  ...
  // Wing is fixed for all except Super Admin
  const lockWing       = isVolunteer || isTeamLead || isAdmin;
  // Committee is fixed for Volunteer & TL; Admin can choose within their wing
  const lockCommittee  = isVolunteer || isTeamLead;
  ...
  // Volunteer can only submit (change status), not approve
  const canSubmitForReview = form.status !== 'Pending Review' && form.status !== 'Completed';
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const handleSave = (submitForReview = false) => {
    const finalStatus = submitForReview ? 'Pending Review' : form.status;
    onSave({
      ...initial,
      ...form,
      status:   finalStatus,
      duration: `${form.hours || 0}h ${form.minutes || 0}m`,
      tlComment: submitForReview ? null : initial?.tlComment,
    });
    onClose();
  };
  ...
  return (
    <Modal title={isEdit ? 'Edit Log' : 'Add Work Log'} onClose={onClose}>
      <div style={{ display: 'grid', gap: '0.875rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          ...
          <div>
            ...
            <select className="input-field" value={form.tag} onChange={e => set('tag', e.target.value)} style={{ cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          <div>
            <label className="input-label">Wing {lockWing && <span style={{ color: 'var(--color-outline)', fontWeight: 400, fontSize: '0.75rem' }}>(assigned)</span>}</label>
            <div style={{ position: 'relative' }}>
              <input className="input-field"
                placeholder="e.g. Tech Wing"
                value={form.wing}
                onChange={e => set('wing', e.target.value)}
                readOnly={lockWing}
                style={{ background: lockWing ? 'var(--color-surface-low)' : undefined, cursor: lockWing ? 'not-allowed' : undefined, paddingRight: lockWing ? '2rem' : undefined }}
              />
              {lockWing && <Lock size={12} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-outline)', pointerEvents: 'none' }} />}
            </div>
          </div>
          <div>
            <label className="input-label">Committee {lockCommittee && <span style={{ color: 'var(--color-outline)', fontWeight: 400, fontSize: '0.75rem' }}>(assigned)</span>}</label>
            <div style={{ position: 'relative' }}>
              <input className="input-field"
                placeholder={isAdmin ? 'Any committee in your wing' : 'e.g. Dev Board'}
                value={form.committee}
                onChange={e => set('committee', e.target.value)}
                readOnly={lockCommittee}
                style={{ background: lockCommittee ? 'var(--color-surface-low)' : undefined, cursor: lockCommittee ? 'not-allowed' : undefined, paddingRight: lockCommittee ? '2rem' : undefined }}
              />
              {lockCommittee && <Lock size={12} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-outline)', pointerEvents: 'none' }} />}
            </div>
          </div>
        </div>
        <div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {VOLUNTEER_STATUSES.map(s => (
              <button key={s} type="button"
                ...
                disabled={s === 'Completed' || (initial?.status === 'Completed')}
                onClick={() => set('status', s)}
                style={{ opacity: (s === 'Completed') ? 0.4 : 1 }}
              >
                {s}
              </button>
            ))}
          </div>
          <p style={{ margin: '0.375rem 0 0', fontSize: '0.75rem', color: 'var(--color-on-surface-variant)' }}>
            {STATUS_META[form.status]?.desc}
          </p>
        </div>
        <div>
        ...
        {/* TL comment (if rejected) — read-only */}
        {initial?.tlComment && (
          <div style={{ padding: '0.75rem', background: 'var(--color-error-container)', borderRadius: '0.5rem', borderLeft: '3px solid var(--color-error)' }}>
            <p style={{ margin: 0, fontSize: '0.75rem', fontWeight: 700, color: 'var(--color-on-error-container)', marginBottom: '0.25rem' }}>TEAM LEAD FEEDBACK</p>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
        ...
        <button type="button" className="btn-secondary" onClick={() => handleSave(false)} style={{ flex: 1, justifyContent: 'center' }}>
          <Save size={14} /> Save Draft
        </button>
        {canSubmitForReview && (
          <button type="button" className="btn-primary" onClick={() => handleSave(true)} style={{ flex: 1.2, justifyContent: 'center' }}>
  ...
  <Modal title="Log Details" onClose={onClose}>
    <div style={{ display: 'grid', gap: '1rem' }}>
      <div style={{ padding: '1rem', background: 'var(--color-surface-low)', borderRadius: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <span className={`badge ${tagBadge[log.tag] || 'badge-neutral'}`}>{log.tag}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
        {[
          ...
          <div key={label}>
            ...
            <p style={{ margin: '0.2rem 0 0', fontSize: '0.9375rem', fontWeight: 500 }}>{value}</p>
          </div>
        ))}
      </div>
      ...
      {log.notes && (
        <div>
          <p style={{ margin: '0 0 0.375rem', fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-on-surface-variant)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Notes</p>
          <p style={{ margin: 0, padding: '0.75rem', background: 'var(--color-surface-low)', borderRadius: '0.5rem', fontSize: '0.875rem', lineHeight: 1.6 }}>{log.notes}</p>
        </div>
      )}
      ...
      {log.tlComment && (
        <div style={{
          padding: '0.75rem',
          background: log.status === 'Completed' ? 'var(--color-surface-low)' : 'var(--color-error-container)',
          borderRadius: '0.5rem',
          borderLeft: `3px solid ${log.status === 'Completed' ? '#059669' : 'var(--color-error)'}`,
        }}>
          <p style={{ margin: '0 0 0.25rem', fontSize: '0.75rem', fontWeight: 700, color: log.status === 'Completed' ? 'var(--color-primary)' : 'var(--color-on-error-container)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            Team Lead {log.status === 'Completed' ? 'Approval' : 'Feedback'}
          </p>
          <p style={{ margin: 0, fontSize: '0.875rem', color: log.status === 'Completed' ? 'var(--color-on-surface)' : 'var(--color-on-error-container)' }}>{log.tlComment}</p>
        </div>
      )}
    </div>
...
const ApproveModal = ({ log, onClose, onApprove }) => {
  ...
  return (
    <Modal title={`Approve: ${log.title}`} onClose={onClose} maxWidth="460px">
      <div style={{ padding: '0.75rem', background: 'var(--color-surface-low)', borderRadius: '0.5rem', marginBottom: '1rem' }}>
        <p style={{ margin: 0, fontSize: '0.8125rem', fontWeight: 600 }}>{log.submitter} · {log.duration} · {log.date}</p>
        <p style={{ margin: '0.25rem 0 0', fontSize: '0.8125rem', color: 'var(--color-on-surface-variant)' }}>{log.wing} · {log.committee}</p>
      </div>
      <div style={{ marginBottom: '1.25rem' }}>
        <label className="input-label">Approval Note (optional)</label>
        <textarea className="input-field" rows={3} placeholder="Add a comment for the volunteer…" value={comment} onChange={e => setComment(e.target.value)} style={{ resize: 'vertical' }} />
      </div>
      <div style={{ display: 'flex', gap: '0.75rem' }}>
        <button className="btn-secondary" onClick={onClose} style={{ flex: 1 }}>Cancel</button>
        <button className="btn-primary" onClick={() => { onApprove(log.id, 'Completed', comment || 'Approved.'); onClose(); }} style={{ flex: 1, justifyContent: 'center', background: 'linear-gradient(135deg,#059669,#047857)' }}>
...
const RejectModal = ({ log, onClose, onReject }) => {
  const [comment, setComment] = useState('');
  return (
    <Modal title={`Request Revision: ${log.title}`} onClose={onClose} maxWidth="460px">
      <div style={{ padding: '0.75rem', background: 'var(--color-surface-low)', borderRadius: '0.5rem', marginBottom: '1rem' }}>
        <p style={{ margin: 0, fontSize: '0.8125rem', fontWeight: 600 }}>{log.submitter} · {log.duration} · {log.date}</p>
      </div>
      <div style={{ marginBottom: '1.25rem' }}>
        <label className="input-label">Reason / Feedback *</label>
        <textarea className="input-field" rows={4} placeholder="Explain what the volunteer needs to fix or add…" value={comment} onChange={e => setComment(e.target.value)} style={{ resize: 'vertical' }} required />
      </div>
      <div style={{ display: 'flex', gap: '0.75rem' }}>
        <button className="btn-secondary" onClick={onClose} style={{ flex: 1 }}>Cancel</button>
        <button disabled={!comment.trim()} onClick={() => { onReject(log.id, 'Needs Revision', comment); onClose(); }} style={{ flex: 1, padding: '0.5rem 1rem', background: comment.trim() ? 'var(--color-error)' : 'var(--color-surface-high)', color: comment.trim() ? '#fff' : 'var(--color-outline)', border: 'none', borderRadius: '0.5rem', fontWeight: 600, cursor: comment.trim() ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem' }}>
...
const ConfirmDelete = ({ log, onConfirm, onClose }) => (
  <Modal title="Delete Log" onClose={onClose} maxWidth="400px">
    <p style={{ fontSize: '0.875rem', color: 'var(--color-on-surface-variant)', margin: '0 0 1.5rem' }}>
      Delete <strong style={{ color: 'var(--color-on-surface)' }}>"{log.title}"</strong>? This action cannot be undone.
    </p>
    <div style={{ display: 'flex', gap: '0.75rem' }}>
      <button className="btn-secondary" onClick={onClose} style={{ flex: 1 }}>Cancel</button>
...
const Logs = () => {
  ...
  const scopeLabel = role === ROLES.VOLUNTEER ? 'My Work Logs'
    : role === ROLES.TEAM_LEAD ? 'Committee Logs'
    : role === ROLES.ADMIN ? 'Wing Logs' : 'All Logs';

  /* Volunteer status guide */
  ...
  return (
    <>
      ...
      <div className="page-body">
        ...
        {/* Volunteer: Needs Revision alert */}
        {role === ROLES.VOLUNTEER && needsRevisionLogs.length > 0 && (
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', padding: '0.875rem 1rem', background: 'var(--color-error-container)', borderRadius: '0.625rem', marginBottom: '1rem', borderLeft: '4px solid var(--color-error)' }}>
            ...
            <div>
              <p style={{ margin: 0, fontWeight: 700, fontSize: '0.875rem', color: 'var(--color-on-error-container)' }}>
                {needsRevisionLogs.length} log{needsRevisionLogs.length > 1 ? 's' : ''} need{needsRevisionLogs.length === 1 ? 's' : ''} your attention
              </p>
              <p style={{ margin: '0.2rem 0 0', fontSize: '0.8125rem', color: 'var(--color-on-error-container)' }}>
                Your Team Lead has requested revisions. Edit the flagged logs and resubmit for review.
              </p>
            </div>
          </div>
        )}

        {/* TL: Pending Review banner */}
        {role === ROLES.TEAM_LEAD && pending > 0 && (
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', padding: '0.875rem 1rem', background: 'var(--color-secondary-fixed)', borderRadius: '0.625rem', marginBottom: '1rem', borderLeft: '4px solid #674db0' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.875rem', color: '#4e3397', fontWeight: 600 }}>
              {pending} log{pending > 1 ? 's' : ''} pending your review &nbsp;·&nbsp;
              <button onClick={() => setFilter('Pending Review')} style={{ background: 'none', border: 'none', padding: 0, color: '#674db0', fontWeight: 700, cursor: 'pointer', textDecoration: 'underline' }}>
                Review now
              </button>
            </p>
          </div>
        ...
        {role === ROLES.VOLUNTEER && (
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', padding: '0.625rem 0.875rem', background: 'var(--color-surface-low)', borderRadius: '0.5rem', marginBottom: '1rem', fontSize: '0.8125rem', color: 'var(--color-on-surface-variant)' }}>
            ...
            <span>Workflow: <strong>Draft → In Progress → Submit for Review → Team Lead approves → Completed</strong></span>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
            {filterOptions.map(f => (
              ...
              >
                {f}
                {f === 'Pending Review' && pending > 0 && (
                  <span style={{ position: 'absolute', top: '-5px', right: '-5px', width: '16px', height: '16px', background: 'var(--color-error)', color: '#fff', borderRadius: '9999px', fontSize: '0.625rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--color-background)' }}>
                    {pending}
                  </span>
                )}
              </button>
          ...
          {['Draft', 'In Progress', 'Pending Review', 'Needs Revision', 'Completed'].map(s => {
            ...
            return (
              <button key={s} onClick={() => setFilter(filter === s ? 'All' : s)}
                className="card"
                style={{ padding: '0.75rem 1rem', cursor: 'pointer', border: filter === s ? '2px solid var(--color-primary)' : '2px solid transparent', textAlign: 'left', minWidth: '5rem' }}>
        ...
        <div className="desktop-log-table card" style={{ padding: 0, overflow: 'hidden' }}>
          <table className="data-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '3rem', color: 'var(--color-on-surface-variant)' }}>
                  No logs for "{filter}"{search ? ` matching "${search}"` : ''}.
                </td></tr>
              ) : filtered.map(log => (
                <tr key={log.id} style={{ background: log.status === 'Needs Revision' && log.isOwn ? 'var(--color-error-container)' : undefined, opacity: log.status === 'Needs Revision' && log.isOwn ? 0.92 : 1 }}>
                  {role !== ROLES.VOLUNTEER && (
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <div className="avatar" style={{ width: '1.625rem', height: '1.625rem', fontSize: '0.6rem', flexShrink: 0 }}>{log.submitter[0]}</div>
                  ...
                  <td>
                    <span style={{ fontWeight: 500, display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                      {log.title}
                      {log.status === 'Needs Revision' && log.isOwn && <AlertCircle size={13} style={{ color: 'var(--color-error)', flexShrink: 0 }} />}
                    </span>
                  </td>
                  ...
                  <td>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.8125rem' }}>
                      <Clock size={12} style={{ color: 'var(--color-outline)' }} />{log.duration}
                    </span>
                  </td>
                  <td><span className={`badge ${tagBadge[log.tag] || 'badge-neutral'}`}>{log.tag}</span></td>
                  {role !== ROLES.VOLUNTEER && <td style={{ fontSize: '0.8125rem', color: 'var(--color-on-surface-variant)' }}>{log.committee}</td>}
                  <td><StatusBadge status={log.status} /></td>
                  {role === ROLES.TEAM_LEAD && (
                    <td style={{ maxWidth: '160px' }}>
                      {log.tlComment ? (
                        <span style={{ fontSize: '0.75rem', color: 'var(--color-on-surface-variant)', fontStyle: 'italic', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>{log.tlComment}</span>
                      ) : <span style={{ fontSize: '0.75rem', color: 'var(--color-outline)' }}>—</span>}
                    </td>
                  )}
                  <td>
                    <div style={{ display: 'flex', gap: '0.25rem', justifyContent: 'flex-end', alignItems: 'center' }}>
                      ...
                      {log.isOwn && log.status !== 'Completed' && (
                        <button title="Edit" onClick={() => setEdit(log)} style={{ padding: '0.3rem 0.5rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-primary)', display: 'flex', alignItems: 'center', borderRadius: '0.375rem' }}><Edit2 size={14} /></button>
                      )}
                      {can(role, 'approveLog') && !log.isOwn && log.status === 'Pending Review' && (
                        <>
                          <button title="Approve" onClick={() => setApprove(log)} style={{ padding: '0.3rem 0.5rem', background: 'none', border: 'none', cursor: 'pointer', color: '#059669', display: 'flex', alignItems: 'center', borderRadius: '0.375rem' }}><CheckCircle size={14} /></button>
                      ...
                      {log.isOwn && (log.status === 'Draft' || log.status === 'Needs Revision') && (
                        <button title="Delete" onClick={() => setDel(log)} style={{ padding: '0.3rem 0.5rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-error)', display: 'flex', alignItems: 'center', borderRadius: '0.375rem' }}><Trash2 size={14} /></button>
                      )}
                    </div>
                  </td>
                </tr>
          ...
          {filtered.length === 0 ? (
            ...
            <div key={log.id} className={`log-card${log.status === 'Needs Revision' && log.isOwn ? ' needs-revision' : ''}`}>
              ...
              <div className="log-card-meta">
                ...
                {role !== ROLES.VOLUNTEER && <span style={{ fontWeight: 500 }}>{log.submitter}</span>}
              </div>
              <div className="log-card-footer">
                ...
                <div className="log-card-actions">
                  ...
                  {log.isOwn && log.status !== 'Completed' && (
                    <button title="Edit" onClick={() => setEdit(log)} style={{ padding: '0.375rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-primary)', borderRadius: '0.375rem' }}><Edit2 size={15} /></button>
                  )}
                  {can(role, 'approveLog') && !log.isOwn && log.status === 'Pending Review' && (
                    <>
                      <button title="Approve" onClick={() => setApprove(log)} style={{ padding: '0.375rem', background: 'none', border: 'none', cursor: 'pointer', color: '#059669', borderRadius: '0.375rem' }}><CheckCircle size={15} /></button>
                      <button title="Reject" onClick={() => setReject(log)} style={{ padding: '0.375rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-error)', borderRadius: '0.375rem' }}><XCircle size={15} /></button>
                    </>
                  )}
                  {log.isOwn && (log.status === 'Draft' || log.status === 'Needs Revision') && (
                    <button title="Delete" onClick={() => setDel(log)} style={{ padding: '0.375rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-error)', borderRadius: '0.375rem' }}><Trash2 size={15} /></button>
                  )}
                </div>
              </div>
            </div>
        ...
        {/* FAB — mobile Add Log */}
        <button className="fab" onClick={() => setAdd(true)} title="Add Log" aria-label="Add Log">
          <PlusCircle size={22} />
        </button>
